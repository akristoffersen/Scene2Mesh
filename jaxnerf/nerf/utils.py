...
def render_image(render_fn, rays, rng, normalize_disp, chunk=8192):
  ...
  Returns:
    ...
    sigma: jnp.ndarray, sigma value per pixel.
  """
  ...
  rgb, disp, acc, sigma, raw_rgb = [jnp.concatenate(r, axis=0) for r in zip(*results)]
  ...
  return (rgb.reshape((height, width, -1)), disp.reshape(
      (height, width, -1)), acc.reshape((height, width, -1)), sigma.reshape((height, width, -1)), raw_rgb.reshape((height, width, -1)))
